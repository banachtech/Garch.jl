...
export simulate, garch, garchZD, fitBayes, fitMLE, garchGJR
...
"""
function simulate(a, b, c; y₀ = 0.001, h₀ = 0.0, n = 100, d = Normal(0.0, 1.0))
...
"""
    garchGJR(y::Vector{Float64}, z::Vector{Float64})

Model GJR-Garch(1,1) for Bayesian estimation.
"""
@model function garchGJR(y, z)
    n = length(y)
    h = 0.01
    a1 ~ Uniform(0.0001, 10.0)
    a2 ~ Uniform(0.0001, 10.0)
    β ~ Uniform(0.0001, 10.0)
    a0 ~ Uniform(0.0, 10.0)
    δ ~ Uniform(0.0001, 10.0)
    y[1] ~ Normal(y[1], h)
    for i = 2:n
        j = y[i-1] < 0.0 ? 1 : 0
        h = a0 + (a1 + j * a2) * y[i-1] * y[i-1] + β * h + δ * z[i-1]
        y[i] ~ Normal(0, sqrt(h))
    end
end
